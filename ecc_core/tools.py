...
- 오래 걸리는 스캔/빌드는 background=true로 띄우고 다른 작업을 병행해라
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "background": {
                    "type": "boolean",
                    "description": (
                        "true이면 명령을 백그라운드 스레드에서 실행하고 즉시 task_id를 반환한다. "
                        "결과는 bash_wait(task_id=...)로 나중에 수집. "
                        "서브넷 스캔, 긴 빌드 등 수십 초 이상 걸리는 작업에 사용."
                    ),
                    "default": False
                },
                "description": {
...
bash(background=true) 호출 후 반환된 task_id를 넘기면 결과를 기다렸다가 반환.
이미 완료된 task는 즉시 반환. timeout 초 내 미완료 시 부분 결과 반환 후 재호출 가능.

패턴 — 병렬로 여러 긴 작업 동시 실행:
  1. t1 = bash(command="네트워크 스캔 ...", background=true, timeout=90)   → task_id 반환
  2. t2 = bash(command="패키지 설치 ...", background=true, timeout=120)   → task_id 반환
  3. bash(command="다른 빠른 작업")                                        → 동기 실행
  4. bash_wait(task_id=t1, timeout=90)   → 스캔 결과 수집
  5. bash_wait(task_id=t2, timeout=120)  → 설치 결과 수집

timeout 가이드:
  - 서브넷 스캔 (254 IPs): timeout=90
  - pip/apt 설치: timeout=120~180
  - ROS2 빌드: timeout=300
  - 타임아웃 후 재호출 가능 (task는 백그라운드에서 계속 실행 중)""",
        "input_schema": {
            "type": "object",
            "properties": {
                "task_id": {
                ...
                "timeout": {
                    "type": "integer",
                    "description": "최대 대기 시간(초). 기본 120. 작업 특성에 맞게 명시할 것.",
                    "default": 120
                },
                "description": {
                    "type": "string",
...
- camera:        카메라 디바이스
- lidar:         LiDAR 센서
- parallel_scan: 보드에서 동일 네트워크의 모든 디바이스를 병렬 스캔
                 bash(background=true)와 달리 probe 자체에서 병렬 ping을 실행.
                 254개 IP를 30초 이내에 스캔. 이더넷 LiDAR, 카메라, 센서 IP 탐색에 사용.
...
probe 결과에서 확인된 값을 사용해라.""",
    ...
    # ── 8. serial_open ─────────────────────────────────────────
    # CC v2의 open_serial에 해당 — 보드에 붙은 MCU/센서를 직접 제어
    {
        "name": "serial_open",
        "description": """보드에 연결된 시리얼 장치(Arduino, VESC, Dynamixel 등)와 통신 세션을 연다.

probe(target="hw")나 probe(target="motors")로 포트를 확인한 다음 사용.

반환값: session_id (serial_send/serial_close에 사용)

사용 패턴:
  1. probe(target="hw") → /dev/ttyACM0 확인
  2. serial_open(port="/dev/ttyACM0", baudrate=115200) → session_id 반환
  3. serial_send(session_id=..., data="AT\\r\\n", expect="OK") → 응답 확인
  4. serial_send(session_id=..., data="FWD 100 5\\r\\n") → 명령 전송
  5. serial_close(session_id=...)

bash/script로 pyserial 직접 쓰는 것보다 이 도구를 써야 하는 이유:
- 세션 상태가 유지됨 (연속 명령 전송 가능)
- expect 패턴으로 응답 자동 대기
- IORecord에 송수신 히스토리 자동 기록""",
        "input_schema": {
            "type": "object",
            "properties": {
                "port": {
                    "type": "string",
                    "description": "시리얼 포트 경로. 예: /dev/ttyACM0, /dev/ttyUSB0"
                },
                "baudrate": {
                    "type": "integer",
                    "description": "통신 속도. 일반값: 9600, 115200, 256000, 460800",
                    "default": 115200
                },
                "timeout": {
                    "type": "number",
                    "description": "읽기 타임아웃(초). 기본 1.0",
                    "default": 1.0
                },
                "description": {
                    "type": "string",
                    "description": "이 장치가 무엇인지 (예: VESC 모터 컨트롤러)"
                }
            },
            "required": ["port"]
        }
...
CC v2의 send_then_receive에 해당. request-response 프로토콜에 최적.

expect 패턴:
- 지정하면: 해당 문자열이 응답에 나타날 때까지 대기 (또는 timeout)
- 미지정: timeout 초 동안 들어오는 모든 바이트 수집

프로토콜 탐색 패턴 (모르는 장치를 처음 만났을 때):
  serial_send(session_id, "\\r\\n")            → 빈 응답으로 에코 확인
  serial_send(session_id, "AT\\r\\n", "OK")   → AT 명령 계열 확인
  serial_send(session_id, "help\\r\\n")        → 도움말 요청
  serial_send(session_id, "?\\r\\n")           → 쿼리 프로토콜 확인
  serial_send(session_id, "\\x00\\x01")        → 바이너리 프로토콜 확인

hex_encode=true: data를 hex 문자열로 해석해서 바이너리 전송
  예: data="FF A5 01 02" → b'\\xff\\xa5\\x01\\x02' 전송""",
        "input_schema": {
            "type": "object",
            "properties": {
                "session_id": {
                    "type": "string",
                    "description": "serial_open이 반환한 session_id"
                },
                "data": {
                    "type": "string",
                    "description": "전송할 데이터. 줄바꿈은 \\r\\n 또는 \\n으로 명시."
                },
                "expect": {
                    "type": "string",
                    "description": "응답에서 기다릴 문자열. 미지정 시 timeout까지 수집.",
                    "default": ""
                },
                "timeout": {
                    "type": "number",
                    "description": "응답 대기 최대 시간(초). 기본 2.0",
                    "default": 2.0
                },
                "hex_encode": {
                    "type": "boolean",
                    "description": "true면 data를 hex 바이트열로 파싱해서 전송. 예: 'FF A5 01'",
                    "default": False
                }
            },
    ...
    {
        "name": "serial_close",
        "description": """시리얼 세션을 닫는다.

작업 완료 후 반드시 호출. 포트를 점유한 채로 두면 다음 연결이 실패한다.
done() 호출 전에 열린 세션은 모두 닫아라.

session_id 없이 호출하면 모든 열린 세션을 닫는다.""",
        "input_schema": {
            "type": "object",
            "properties": {
                "session_id": {
                    "type": "string",
                    "description": "닫을 session_id. 미지정 시 전체 닫기."
                }
            },
            "required": []
        }
...
⚠️  실행(제어, 설정, 물리 동작)은 메인 에이전트가 직접 bash/script로 해야 한다.
subagent는 "뭐가 있는지 알아봐줘" 전용이다. "해줘"가 아니다.
...
잘못된 사용:
  ...
  ❌ subagent("ROS2를 실행하고 결과를 확인해라")  → 메인이 직접 bash()로 해라
  ❌ 연결 직후 바로 subagent("모든 걸 탐색해라")  → probe(target="all")이면 충분

언제 써야 하는가:
- 결과를 예측할 수 없는 탐색 (이 보드에 어떤 X가 있는가?)
- 대량의 파일/로그 분석 (수십 개 파일에서 패턴 찾기)
- 메인 컨텍스트가 오염될 만큼 많은 시도-실패가 예상되는 초기 조사
...
호출 조건:
- 물리 동작 goal: 하드웨어 readback으로 실제 효과 확인 후 호출
- 탐색/분석 goal: 정보 수집 완료 후 호출
...
물리 동작 확인 방법 (done 호출 전 반드시 수행):
  모터:   ERPM/속도 텔레메트리 읽기, 또는 ros2 topic echo
  센서:   실제 데이터 수신 확인
  통신:   응답 패킷 확인
  ROS2:   topic hz / topic echo로 실제 publish 확인
...
물리 한계 발견 시: success=false + 실제로 가능한 대안 제시.""",
        "input_schema": {
            ...
            "properties": {
                ...
                "summary": {
                    "type": "string",
                    "description": "무엇을 했고 결과가 어떻게 됐는지. 수치 포함 (예: actual speed=0.09 m/s, ERPM=415)."
                },
                "evidence": {
                    "type": "string",
                    "description": (
...
# parallel_scan: 보드에서 동일 서브넷의 모든 IP를 병렬 ping — CC TaskOutput(block=False) 방식과 동일
# 254개 IP를 백그라운드 병렬로 쏘고 응답 수집. nmap 없어도 동작.
...
if [ -z "$SUBNETS" ]; then
  echo "서브넷 감지 실패: 네트워크 인터페이스 없음"
  exit 1
fi

for SUBNET in $SUBNETS; do
  echo "--- 스캔: ${SUBNET}.0/24 ---"
  # 254개 병렬 ping (백그라운드 per-IP)
  FOUND=""
  for i in $(seq 1 254); do
    (ping -c 1 -W 1 ${SUBNET}.${i} >/dev/null 2>&1 && echo "${SUBNET}.${i}") &
  done
  wait
  echo "스캔 완료: ${SUBNET}.0/24"
done | grep -E "^[0-9]" | sort -t. -k4 -n

echo ""
echo "=== ARP 캐시 (응답한 장치 + MAC) ==="
